...
const organizerSignupRequest = async (req, res, next) => {
  try {
    ...
    await authService.initiateOrganizerSignup(
      name,
      email,
      password,
      {
    ...
    logger.error("Organizer signup OTP request failed", {
      email: req.body?.email,
      error: err.message,
    });
...
const organizerSignupVerify = async (req, res, next) => {
  try {
    ...
    logger.info("Organizer signup OTP verification attempted", {
      email,
    });

    const { user, profile } =
      await authService.completeOrganizerSignup(email, otp);
    ...
    res.status(201).json({
      message:
        "Registration successful. Your organizer account is pending admin approval. You will receive an email once reviewed.",
      status: "pending",
    });
  } catch (err) {
    logger.error("Organizer signup OTP verification failed", {
      email: req.body?.email,
      error: err.message,
    });
